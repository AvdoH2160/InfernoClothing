...
const Header2 = () => {
    ...
    const klikNaMenu = () => {
      ...
      setIsSearchVisible(!isSearchVisible); // Hide search when menu opens
    };
    ...
    const izgubljenFocus = () => {
        if (!isExpanded) {
          setIsSearchVisible(false);
          searchBarRef.current?.classList.remove("activated");
          searchOptionRef.current?.classList.remove("activated");
        }
    };
  
    return (
        <header id="header-container" className="collapsed" style={{height:isExpanded ? "500px" : "70px"}}>
            <div id="menu-options">
                <div id="menu-option" className="menu-options" onClick={klikNaMenu}>
                    <div id="rectangle-1" className={isExpanded ? "rotated" : ""}></div>  
                    <div id="rectangle-2" className={isExpanded ? "rotated" : ""}></div>
                    <div id="rectangle-3" className={isExpanded ? "rotated" : ""}></div>     
                </div>
                <div id="search-option" ref={searchOptionRef} className={isSearchVisible ? "activated" : ""} onClick={klikNaSearch}>
                ...
                {/* {isSearchVisible && (
                 <div id="search-field">
                <input type="text" id="searchBar" onBlur={izgubljenFocus} placeholder="Search..."/>
                </div>
                )} */}
                <div id="search-field" style={{visibility:isSearchVisible ? "visible" : "hidden"}}>
                        <input type="text" id="searchBar" onBlur={izgubljenFocus} placeholder="Search..." className={isSearchVisible ? "activated" : ""}/>
                </div>
                <div id="logo-option">
